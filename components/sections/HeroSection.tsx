...
import { getProjectBySlug, heroImages } from "@/lib/data";
...
const carouselItems = heroImages.map((item) => ({
  slug: item.slug,
  src: item.src,
  alt: item.alt,
  title: item.title,
  tools: getProjectBySlug(item.slug)?.tools ?? [],
}));
        ...
        <div className="grid gap-9 lg:grid-cols-[1fr_0.95fr] lg:items-end">
                ...
                <span className="relative inline-block">
                  ...
                  <span className="group absolute left-[calc(100%+30px)] top-[6px] z-[30] sm:left-[calc(100%+12px)] sm:top-[10px] lg:left-[calc(100%+14px)] lg:top-[12px]">
                    ...
                    <span className="relative z-10 inline-block h-[72px] w-[72px] overflow-hidden rounded-[20px] border border-white shadow-[0_8px_20px_rgba(0,0,0,0.08)] transition-transform duration-200 ease-in-out group-hover:-translate-y-[1px] sm:h-[82px] sm:w-[82px] sm:rounded-[22px]">
                      <Image src="/images/avatar/me.avif" alt="Agustin A. icon" fill className="object-cover" sizes="82px" priority />
                    </span>
                  </span>
                </span>
                <br />
          ...
          <div className="reveal-up reveal-delay-1 max-w-[560px] lg:justify-self-end">
            <p className="reveal-text mb-4 flex flex-wrap items-center gap-3 text-[#5f5f63]">
              ...
              <span className="text-[14px] font-normal text-[#727276]">Helped 7+ businesses & counting</span>
            </p>
            <p className="reveal-text reveal-delay-1 text-[20px] font-normal leading-[1.5] text-[#3f3f45]">
              Full-stack software engineer (BSc in Computer Science) with 4+ years of experience building scalable web
              and automation systems.
            </p>

            <div className="mt-8 flex flex-wrap items-center gap-3 sm:gap-4">
              <a href="#contact" className="btn-primary inline-flex items-center gap-3 rounded-full px-7 py-3.5 text-[16px] font-medium text-white sm:px-9 sm:py-4">
              ...
              <a href="#projects" className="button-soft rounded-full bg-[#f8f8f7] px-7 py-3.5 text-[16px] font-medium text-[#2f2f34] sm:px-8 sm:py-4">
                See Projects
              </a>
            </div>
          </div>
