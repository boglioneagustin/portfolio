...
type ProjectDetailSectionEsProps = {
...
function ToolLogo({ tool }: { tool: string }) {
  ...
  if (key === "php") return <SiPhp aria-hidden="true" className="text-[20px] text-[#777bb3]" />;
  if (key === "codeigniter") return <SiCodeigniter aria-hidden="true" className="text-[20px] text-[#ee4623]" />;
  if (key === "figma") return <SiFigma aria-hidden="true" className="text-[19px] text-[#f24e1e]" />;
  if (key === "framer") return <SiFramer aria-hidden="true" className="text-[19px] text-[#111111]" />;
  if (key === "next.js" || key === "nextjs") return <SiNextdotjs aria-hidden="true" className="text-[18px] text-[#111111]" />;
  if (key === "node.js" || key === "nodejs") return <SiNodedotjs aria-hidden="true" className="text-[18px] text-[#5fa04e]" />;
  if (key === "n8n") return <SiN8N aria-hidden="true" className="text-[18px] text-[#ea4b71]" />;
  if (key === "apify")
    return (
      <Image
  ...
  if (key === "excel" || key === "microsoft excel") return <RiFileExcel2Fill aria-hidden="true" className="text-[19px] text-[#1d6f42]" />;
  if (key === "html") return <SiHtml5 aria-hidden="true" className="text-[19px] text-[#e34f26]" />;
  if (key === "css") return <SiCss aria-hidden="true" className="text-[19px] text-[#1572b6]" />;
  if (key === "javascript" || key === "js") return <SiJavascript aria-hidden="true" className="text-[19px] text-[#f7df1e]" />;
  if (key === "sql" || key === "mysql") return <SiMysql aria-hidden="true" className="text-[19px] text-[#00758f]" />;
  if (key === "react") return <SiReact aria-hidden="true" className="text-[19px] text-[#61dafb]" />;
  if (key === "typescript") return <SiTypescript aria-hidden="true" className="text-[19px] text-[#3178c6]" />;
  if (key === "python") return <SiPython aria-hidden="true" className="text-[19px] text-[#3776ab]" />;
...
function placeholderToneClass(tone?: "dark" | "warm" | "neutral") {
  if (tone === "warm") return "bg-[linear-gradient(125deg,#2a1f1a_0%,#5d3b2d_42%,#241c20_100%)]";
  if (tone === "neutral") return "bg-[linear-gradient(125deg,#d6d6d2_0%,#ccccca_48%,#e2e2de_100%)]";
...
function CaseMedia({
  ...
  return (
    <div className={`relative overflow-hidden rounded-[32px] ${placeholderToneClass(tone)} ${className}`}>
      {imageSrc ? (
        <Image src={imageSrc} alt={imageAlt ?? "Imagen del proyecto"} fill className="object-cover" sizes={sizes} />
      ) : (
        <>
...
export default function ProjectDetailSectionEs({ project }: ProjectDetailSectionEsProps) {
  const relatedProjects = allWorksEs.filter((item) => project.relatedSlugs.includes(item.slug)).slice(0, 2);
  const ctaLabel = project.ctaLabel ?? "Contacto";
          ...
          <dl className="divide-y divide-black/7">
            ...
            {project.tools && project.tools.length > 0 ? (
              <div className="grid grid-cols-1 gap-2 py-4 text-left md:grid-cols-[1fr_260px] md:items-center md:gap-3 md:py-5">
                <dt className="text-[18px] font-normal leading-[1.3] tracking-[0] text-[#91919a] sm:text-[20px]">Herramientas</dt>
                <dd>
        ...
        <div className="mt-16 sm:mt-24">
          ...
          <div className="reveal-up reveal-delay-2 mt-10 text-center">
            <p className="section-description mx-auto max-w-[760px] text-[18px] leading-[1.35] sm:text-[24px] sm:leading-[1.3]">
              Proyectos seleccionados que muestran mi enfoque en diseno, desarrollo y ejecucion.
            </p>
            <Link
              href="/es/all-works"
